...
system_setting,modelscope_setting, ollama_setting, openai_setting  = st.tabs(["系统设置","Modelscope设置","Ollama设置","OpenAI设置"])

with system_setting:
    llm_mode_seletor = st.radio("LLM模式", ["Ollama", "OpenAI"],index=["Ollama", "OpenAI"].index(config['SYSTEM']['llm_mode']))
    if st.button("保存系统设置"):
        try:
...
with ollama_setting:
    ...
    ollama_max_tokens = st.text_input("Ollama Tokens上限",config['OLLAMA']['max_tokens'])
    ollama_temperature = st.slider("Ollama Temperature",0.0,1.0,float(config['OLLAMA']['temperature']))
    ollama_top_p = st.slider("Ollama Top_p",0.0,1.0,float(config['OLLAMA']['top_p']))
...
with openai_setting:
    ...
    def model_manager():
        ...
        if st.button("保存模型信息"):
            ...
            is_valid = all(
                all(value is not None and str(value).strip() != "" for value in item.values())
    ...
    openai_max_tokens = st.text_input("OpenAI Tokens上限",config['OPENAI']['max_tokens'])
    openai_temperature = st.slider("OpenAI Temperature",0.0,1.0,float(config['OPENAI']['temperature']))
    openai_top_p = st.slider("OpenAI Top_p",0.0,1.0,float(config['OPENAI']['top_p']))
